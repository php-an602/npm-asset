...
const imagePlugin = (context) => {
    ...
    context.editor.$.on('mouseleave', e => {
        ...
        if(!$(target).closest('.an602-richtext-inline-menu').length) {
          $('.an602-richtext-inline-menu').remove();
        }
    });
    ...
    return new Plugin({
        ...
        filterTransaction: (tr, state) => {
            if(!(tr.curSelection instanceof NodeSelection)) {
             $('.an602-richtext-image-edit').remove();
            }
...
class ImageView {
    constructor(node, context) {
        ...
        this.dom.addEventListener("mouseenter", e => {
            ...
            let $edit = $('<div>').addClass('an602-richtext-inline-menu').addClass('an602-richtext-image-edit')
