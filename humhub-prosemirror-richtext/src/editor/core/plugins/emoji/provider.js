...
let findUserFlag = function() {
    ...
    try {
        let language = an602.require('user').getLocale().toLowerCase();
...
class EmojiChooser {
    ...
    update(provider, focus) {
        ...
        if(!an602.require('ui.view').isSmall()) {
            this.$.css({
        ...
        if(focus) {
            this.$.find('.an602-emoji-chooser-search').focus();
        }
    }

    initDom() {
        let that = this;
        this.$ = $('<div class="atwho-view an602-richtext-provider an602-emoji-chooser"><div><input type="text" class="form-control an602-emoji-chooser-search"></div></div>')
            .hide().appendTo($('body'))
            .on('hidden', () => {
                if(that.provider) {
                    that.provider.reset();
        ...
        this.$.find('.an602-emoji-chooser-search').on('keydown', function(e) {
            switch (e.which) {
                case 9:
                    e.preventDefault();
                    that.nextCategory();
    ...
    clearSearch() {
        ...
        this.$.find('.an602-emoji-chooser-search').val('');
    }
    ...
    initCategory(categoryName) {
        ...
        $('<ul class="atwo-view-ul an602-emoji-chooser-item-list">').appendTo($category);
    ...
    setCategoryItems(categoryName, items) {
        ...
        let $list = this.categories[categoryName].$.find('.an602-emoji-chooser-item-list').empty();
