...
MentionProvider.prototype.update = function(loading) {
    if(!this.$container) {
        this.$container = $('<div class="atwho-view an602-richtext-provider">').css({'margin-top': '5px'});
    } else {
    ...
    if(this.result && this.result.length) {
        ...
        this.result.forEach(function (item) {
            var name =  an602.modules.util.string.encode(item.name);
        ...
        var name =  an602.modules.util.string.encode(this.result.text);
        this.$container.append($('<span>'+name+'</span>'));
    } else if(this.result.loader) {
        let $loader = an602.require('ui.loader').set($('<span>'), {
            span: true,
            size: '8px',
            css: {
                padding: '0px',
